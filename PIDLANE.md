...
| 10 | `pidlane-archief.js` | 30 | sessierapportarchief, AI-rapporthook, TXT/PDF-export, **de Android-terugknop** (`appBack`) — de enige luisteraar op `backButton`, zie §11 01-09; sinds 26-09 drukt hij het ✕ in van het bovenste venster (`_plBovensteSluitKnop`) — en **het venster "Voor de analyse"** (`plVoorAnalyse`, `PL_VOORVRAGEN`, `plMeetcontextPromptLine`): hergebruik van eerdere rapporten én de meetcontext-vragen in één sheet, zie §11 01-09 |
...
| 11a | `pidlane-visueel.js` | 36 | **Slim visueel**, de vijfde weergave: één vaste meter (toerental, snelheid; onderin op één rij koelwater/accu/brandstof; onderboog olie → laaddruk → gaspedaal, met het getal onder de cirkel) en een meldingenvak eronder dat `PLRun.staat()` en de bevindingen leest: één chip per lopende module op een rail, bevindingen als kaart. Boven de meter twee lampjes uit `PLAandrijving` (motor links met de belasting 0104, hybride rechts met het accupercentage 015B), boven de naaf het embleem; een diesel krijgt schaal 0–6000. Zet `body.pl-visueel`, waarmee `pidlane.css` de monitor- en recorderpil, de waakstrook en de aandrijfbalk verbergt. De wijzerplaat is een vaste tekening uit `G`, de meetwaarden sturen via `stand()` alleen hoek, vulling en tekst. Stuurt een pedaal of laaddruk die op deze auto trager meet dan 800 ms van de onderboog, en remt via `remt()` in `pidPollInterval()` de snelle PIDs die niet op de meter staan. Laadt direct na `pidlane-pids.js` |
...
| 13 | `pidlane-totalcheck.js` | 51 | Total Check — volledige voertuigdoorlichting, en de **systeemtest** (`startBasicCheck`): een checklist waarin elke test uit `BSC_TESTS` meet zodra zijn situatie (`sit`, zie `BSC_SIT`) zich voordoet |
